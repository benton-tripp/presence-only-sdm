...
### Latitude and Longitude as Rasters
...
for (state in states) {
  ...
  if (!file.exists(output.file.lon) | !file.exists(output.file.lat)) {
    # Get raster as df
...
your_data$lon_sq <- st_coordinates(your_data)[,1]^2
your_data$lat_sq <- st_coordinates(your_data)[,2]^2
your_data$lon_lat_interaction <- st_coordinates(your_data)[,1] * st_coordinates(your_data)[,2]
...
model <- lm(dem ~ st_coordinates(your_data)[,1] + 
              st_coordinates(your_data)[,2], data = your_data)
your_data$detrended_dem <- residuals(model)
