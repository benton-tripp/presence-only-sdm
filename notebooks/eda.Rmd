...
r.df[, soil := factor(soil, levels=sort(unique(r.df$soil)))]

# Convert factor columns (except soil) to dummy variables
df.dummies <- data.table(model.matrix(~ . - 1, data = r.df[, .(NLCD_Land, state)])) %>%
  cbind(r.df[, -which(names(r.df) %in% c("NLCD_Land", "state", "soil")), with=F])
...
scaled.data <- scale(df.dummies) %>% as.data.table()

in.df <- cbind(r.df[, .(soil)], scaled.data)

set.seed(19)  # for reproducibility

library(FactoMineR)

mca.fit <- MCA(in.df, quanti.sup=which(names(in.df) != "soil"), graph=F, ncp=4)
