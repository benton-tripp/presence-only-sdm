...
df.train[, `:=` (state=NULL, geometry=NULL)]
df.test <- df[-train.test$index,]
df.test[, `:=` (state=NULL, geometry=NULL)]
...
lasso.list <- purrr::map(species, function(spec) {
  cat("Fitting LASSO model for", spec, "\n")
  spec.df <- train[common.name == spec][, common.name := NULL]
  ...
  fit.l1 <- get.model(
    train(observations ~ (.)^2, 
          ...
          metric = "RMSE"),
    file.name=paste0(spec, ".regr.l1.rds"),
    model.path="models")
...
lasso.imp <- purrr::map(lasso.list, function(fit) {
