...
### Interactions Effects from SHAP
...
#### Get SHAP Values for all Features & Interactions

Steps:

1. Pre-processing: Remove non-predictive variables (`common.name`, `state`, `geometry`). Split the data into training and test sets.
2. Modeling: Fit an XGBoost model.
3. SHAP Analysis: Calculate SHAP values; Explore interaction effects.
...
# Although we could test every single variable, some are slightly redundant.
# Although during variable selection every variable will be tested, that is
# too computationally intesive for SHAP values. Thus, remove some of the 
# unnecessary variables

df[, `:=` (
  dem=NULL, Spring_NDVI=NULL, Summer_NDVI=NULL, Fall_NDVI=NULL,
  Winter_NDVI=NULL, urban_imperviousness=NULL, waterbody=NULL,
  lon.lat.interaction=NULL, lat.sqrd=NULL, lon.sqrd=NULL, 
  hay_pasture=NULL, cultivated_crops=NULL, geometry=NULL,
  developed_open_space=NULL, developed_low_intensity=NULL,
  developed_medium_intensity=NULL, developed_high_intensity=NULL,
  open_water=NULL, emergent_herbaceous_wetlands=NULL, 
  woody_wetlands=NULL
)]
...
# Convert species to binary variables for the purpose of shapley values
# (Can't do that until after the stratified sampling is finished)
binary.spec <- caret::dummyVars(~common.name, data=df, sep="") %>% 
    predict(., df) %>%
    as.data.frame()

names(binary.spec) <- gsub("^common\\.name", "", names(binary.spec)) %>% 
    gsub("\\/| |,", "_", .) %>% 
    gsub("__", "_", .) %>% 
    tolower()

df <- cbind(binary.spec, select(df, -common.name))
df %>% setDT()
...
train.x <- train %>% select(-observations)
train.y <- train$observations

test.x <- test %>% select(-observations)
test.y <- test$observations

```

##### Modeling

```{r inter-shap, results='hide'}
...
explainer <- shapr::shapr(x = train.x, model=xgb.model, n_combinations = 100)
...
plot(explanation, plot_phi0 = F, index_x_test = c(1, 6))
...
SHAP allows the exploration of interaction effects between features. If two features often have non-zero SHAP values together but not individually, they might be interacting in the model.
