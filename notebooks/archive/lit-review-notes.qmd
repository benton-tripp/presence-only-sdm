...
> Modeling species distributions from occurrence records typically demands supplementary data representing a range of environmental conditions in the area, referred to as background or pseudo-absence data. The collection of occurrence data often shows spatial bias, especially favoring easily accessible areas, which can consequently introduce environmental bias. This discrepancy between the occurrence and background data can lead to flawed models. Although accurate spatial bias information is often unavailable, a common bias is likely among species observed using similar methods. An examination involving 226 species worldwide showed that using all occurrences within such a group, referred to as "target-group background", enhances average model performance across different modeling methods. The boost in performance was especially pronounced when the presence records of the target group displayed strong bias. This strategy is applicable to several modeling methods, including generalized linear/additive models, boosted regression trees, and Maxent.
The selection of background data is a crucial step as it should reflect the range of environmental conditions in the region. Some view background samples as indications of where species might be absent, but their primary purpose is to represent the available environmental conditions in the region. The spatial distribution of these records depends on the actual species distribution and survey effort. If there's spatial bias without considering the environment, the resulting model could be more representative of survey effort than the true species distribution. For instance, a species might be mostly recorded near towns and roads, introducing bias if these locations don't represent the entire environmental spectrum. Addressing this bias can involve manipulating occurrence data or surveying underrepresented regions, but these methods have limitations. Instead, manipulating background data to mirror the bias in presence data can be more effective. This ensures the model focuses on genuine differentiation between species occurrences and the background, rather than merely highlighting heavily surveyed areas. However, creating such background data is challenging due to limited knowledge about the exact selection distribution. An alternative approach suggests using presence records from a broader set of species (a "target group") to estimate survey effort, assuming they are collected using similar methods. (Dudı´k et al. 2005; Zaniewski et al. 2002; Engler et al. 2004; Lu¨ tolf et al. 2006; Ferrier et al. 2002; Keating & Cherry 2004; Ponder et al. 2001; Anderson 2003)

> Sample selection bias, though thoroughly examined in other sectors like econometrics (Heckman, 1979), remains under-addressed in species distribution modeling. This bias poses significant challenges for models based on presence-only data, such as those from natural history museums, affecting them more severely than those derived from presence-absence data. Addressing this bias can be achieved when the sampling distribution is known, by using similarly biased background data, a principle applicable to many presence-only modeling techniques, although previously only addressed in specific methods (Argae´z et al. 2005; Dudı´k et al. 2005; Schulman et al. 2007). In cases where the sampling distribution is unknown, approximating it by amalgamating occurrence records for a target group of similarly collected species is recommended. Evaluations on 226 species across four modeling methods showed that the target-group background enhanced predictive performance for all methods, rendering the background data choice as crucial as the modeling method selection for presence-only species distribution models.

**Literature Review**

Phillips et al. (2009b) underscore the significance of background or pseudo-absence data in modeling species distributions from occurrence records. Such data helps to depict the range of environmental conditions in the modeled region. However, a challenge arises due to the spatial bias typically seen in occurrence collection, as this collection often favors easily accessed areas. This spatial bias, in turn, can introduce an environmental bias, potentially resulting in inaccurate models if not appropriately addressed.

A notable observation is that accurate information on this spatial bias is often absent, rendering explicit biased sampling of background sites unfeasible. Nevertheless, the authors posit that species observed using similar methods are likely to share similar biases. To validate this, they delved into an extensive study involving 226 species from diverse global regions, comparing model performance using target-group background and randomly sampled background. The results were illuminating, revealing that target-group background data consistently enhances average model performance across a myriad of modeling techniques, such as generalized linear/additive models, boosted regression trees, and Maxent. This enhancement was particularly pronounced when strong bias characterized the target-group presence records. In essence, the choice of background data exerted as profound an influence on predictive performance as did the choice of the modeling method itself (Phillips et al., 2009b).

On the matter of background data selection, its main role is to typify available environmental conditions within a region. Some researchers consider background samples indicative of potential species absence sites. However, Phillips et al. (2009b) emphasize that the primary objective is to represent the environmental conditions breadth. Addressing the spatial bias is pivotal, as a model influenced by it might lean more towards reflecting survey efforts rather than the actual species distribution. For instance, records predominantly near towns and roads could skew a model if these areas don't encapsulate the entire environmental range. While various strategies, like manipulating occurrence data or increased surveys in underrepresented regions, have been suggested, they come with inherent limitations. A more effective approach involves adjusting background data to match the presence data bias, a tactic backed by several preceding studies (Dudı´k et al. 2005; Zaniewski et al. 2002; Engler et al. 2004; Lu¨ tolf et al. 2006; Ferrier et al. 2002; Keating & Cherry 2004; Ponder et al. 2001; Anderson 2003).

Lastly, the authors highlight a concerning lacuna in species distribution modeling: the under-addressed issue of sample selection bias. This bias, extensively studied in sectors such as econometrics (Heckman, 1979), poses significant ramifications for models grounded in presence-only data. Models derived from sources like natural history museums are more susceptible to this bias compared to those from presence-absence data. A viable solution, where the sampling distribution is known, involves employing correspondingly biased background data. However, when this distribution remains elusive, the researchers advocate for an innovative approach: pooling occurrence records for a target group of species collected similarly. Comprehensive evaluations across four modeling techniques reinforced this approach, emphasizing the pivotal role of background data choice alongside modeling method selection in presence-only species distribution models (Phillips et al., 2009b).
...
Senay, Worner, and Ikeda (2013) delved into the pivotal domain of pseudo-absence selection for spatial distribution models (SDMs). The ongoing exploration of this area has led to the proliferation of varied techniques with incongruous reports regarding their efficacy. Recognizing the inherent value of presence and absence data in ensuring reliable SDM predictions, Senay et al. underscored the need for a technique that judiciously combines geographical and environmental facets to yield pseudo-absence points that are balanced in both dimensions. Consequently, they introduced a novel three-tiered approach to achieve this balance. The first step centers on determining a geographically sensible scope around species presence points, pivoting around an analysis of environmental variables across various distances. This strategy is contrasted against merely adopting an arbitrary radius. The subsequent step identifies locations environmentally divergent from presence points, bound by the distance designated in the first step. Finally, K-means clustering is employed in the third step to trim down potential pseudo-absences, with the cluster centroids in environmentally discordant classes, as identified in the prior step, being taken into consideration.

SDMs, as described by the authors, play a pivotal role in predicting the distribution of species for conservation, biocontrol introductions, and notably, in gauging the potential spread and establishment of invasive species. Despite criticisms, their popularity has surged, attributed to the juxtaposition of enhanced data accessibility and computational advancements ([2,3,4]). SDMs predominantly function by drawing inferences from the environmental niche of species based on known presence locations. Mechanistic or process-driven models, which mandate an in-depth understanding of the species' environmental and physiological prerequisites, often stand as alternatives, albeit they aren't always feasible ([5,6]). A significant critique against SDMs is their potential inaccuracy stemming from the absence of reliable absence information ([7,8]). Senay et al. identify potential pitfalls in deducing true absences, such as undetected species presence or the species' absence due to natural barriers, competition, or even transient migratory patterns.

In addressing the dearth of absence data, Senay et al. discuss three cardinal methodologies: simple presence-only models, enhanced presence-only models, and presence-absence models. While the former solely relies on presence data to demarcate species distribution, the enhanced variant delves deeper by aligning presence-only data with key background environmental variables, exemplified by models like MAXENT, ENFA, and PBL ([3,9,13]). Presence-absence models, on the other hand, use both kinds of data to forecast habitat suitability or species distribution. Notably, in the absence of genuine absence data, pseudo-absence points are often generated. These models range from regression-based ones like GLM and GAM to more novel machine learning tools like ANN, SVM, and naı¨ve Bayes ([15,16]). The authors also expound upon the three chief types of pseudo-absence selection methods which include simple random pseudo-absence selection, those restricted by geographical extent, and those rooted in environmental variables. The latter has recently emerged as a standard given its documented enhanced accuracy ([25,26,32]).

In conclusion, the article by Senay et al. (2013) provides a comprehensive analysis of pseudo-absence selection techniques in SDMs, advocating for a balanced method that melds both spatial and ecological considerations. Their proposed three-step method, which hinges on geography, environment, and clustering techniques, stands as a promising tool to refine the accuracy of SDM predictions.

(Jime´nez-Valverde et al, 2008; Phillips et al., 2006; Lobo et al., 2010; Peterson, 2006; Kearney & Porter, 2009; Wisz & Guisan, 2009; Soberon & Peterson, 2005)

(Hirzel et al., 2002; Li et al., 2011; Elith et al., 2006; Lorena et al., 2011; Chefauoui & Lobo, 2008; Warton & Shepherd, 2010; Hengl, 2009)

Jime´nez-Valverde A., Lobo JM, Hortal J (2008) Not as good as they seem: the importance of concepts in species distribution modelling. Diversity and Distributions 14: 885–890.

Phillips SJ, Anderson RP, Schapire RE (2006) Maximum entropy modeling of species geographic distributions. Ecological Modelling 190: 231–259.

Lobo JM, Jime´nez-Valverde A, Hortal J (2010) The uncertain nature of absences and their importance in species distribution modelling. Ecography 33: 103–114.
...
Wisz M, Guisan A (2009) Do pseudo-absence selection strategies influence species distribution models and their predictions? An information-theoretic approach based on simulated data. BMC Ecology 9: 8.

Soberon J, Peterson AT (2005) Interpretation of models of fundamental ecological niches and species’ distriputional areas. Biodiversity Informatics 2: 1–10.
...
Elith J, Graham CH, Anderson RP, Dudik M, Ferrier S, et al. (2006) Novel methods improve prediction of species; distributions from occurance data. Ecography 29: 129–151.

Lorena AC, Jacintho LFO, Siqueira MF, Giovanni RD, Lohmann LG, et al. (2011) Comparing machine learning classifiers in potential distribution modelling. Expert Systems with Applications 38: 5268–5275.

Chefaoui RM, Lobo JM (2008) Assessing the effects of Pseudo-absence on predictive distribution model performance. Ecological Modelling 210: 478–486.
...
> Random forests (RF) is a powerful species distribution model (SDM) algorithm. This ensemble model by default can produce categorical and numerical species distribution maps based on its classification tree (CT) and regression tree (RT) algorithms, respectively. The CT algorithm can also produce numerical predictions (class probability). Here, we present a detailed procedure involving the use of the CT and RT algorithms using the RF method with presence-only data to model the distribution of species. CT and RT are used to generate numerical prediction maps, and then numerical predictions are converted to binary predictions through objective threshold-setting methods. We also applied simple methods to deal with collinearity of predictor variables and spatial autocorrelation of species occurrence data. A geographically stratified sampling method was employed for generating pseudo-absences. The detailed procedural framework is meant to be a generic method to be applied to virtually any SDM prediction question using presence-only data.

> There is now a plethora of metrics for evaluating SDM performance [3,4]. In short, different accuracy measures have different strengths and weaknesses, and none can provide a universal rating for SDM performance. This phenomenon may be ascribed to the fact that different measures have different strategies of weighting the various types of prediction errors (e.g., omission, commission, or confusion), especially for composite metrics that are based on different algorithms and assumptions (e.g., Kappa; overall accuracy, OA). Therefore, we argue for applying multiple performance metrics to evaluate model performance.
Threshold-independent evaluation (numerical prediction evaluation) For numerical prediction, the predictive performances were evaluated using such measures as the root mean square error (RMSE), the mean absolute prediction error (MAE), the coefficient of determination (R2), mean cross entropy (MXE), and area under the curves (AUCs) of four threshold-independent measures: the area under the sensitivity curve, the area under the specificity curve, the area under the accuracy curve, and the area under the receiver operating characteristic curve (ROC). The latter four measures related to the AUC were estimated using the “AUC” package in the R statistical environment [5]. Measures of AUC avoid the need to choose a threshold value that separates presence from absence (i.e., it is threshold independent) and in addition describe the overall ability of the model to discriminate between two cases.For numerical predictions, accuracy measures often characterize two aspects of SDM models:
discrimination capacity (e.g., AUC values) and reliability (e.g., RMSE, MAE, and R2) [4]. Discrimination capacity measures the ability to discriminate presence and absence based on model predictions. Reliability tells us about how closely predicted probabilities match observed proportions of occurrence, i.e., goodness of fit. The relative importance of reliability and discrimination capacity depends on the use of the model and the experience level of the user [6]. 

> When converting numerical predictions into binary predictions, the optimal threshold varies with the choice of threshold-setting method. However, the choice of thresholds has practical consequences for estimating of RF model performance and species range shifts under climate change [9]. Hence, the use of an appropriate threshold appears to be a better choice for binary conversions for RF. Zhang et al. [9] demonstrated that the four threshold methods (MaxKappa, MaxOA, MinROCdist, and MaxTSS) based on the composite model accuracy measures (Kappa, TSS, ROC, OA) are promising objective methods for binary conversions of continuous predictions when presence-only data are available. These four methods can also produce the same threshold using either presence-only data or presence/absence data for CT and RT models. The top four approaches performed equally well in terms of model performance, threshold determination, and range shift projection, and each often performed better than the other approaches. The CT default classification method (default 0.5) was not recommended for binary conversions [9,10].

> Random forest is an ensemble learning technique. RF by default can yield categorical and numerical species distribution maps based on the classification tree (CT) and regression tree (RT) algorithms, respectively [26]. RF models in the form of CT and RT are commonly and successfully used in species distribution modeling [27]. In statistical terms, CT can also produce probabilistic predictions (class probability) [9]. In RF models, bootstrap samples are applied to construct a large number of decision trees. These trees are then used to predict new data by aggregating the predictions of the trees (i.e., the proportion of votes for classification, or the average for regression; [28]). In a typical CT, the resulting
model output is categorical, and the “winning” class for an observation is the one with the maximum ratio of proportion of votes (the default is 1/k, where k is the number of classes). For presence–absence data, the ratio of the proportions of votes for presence or absence ranges from 0 to 1, and the sum of the ratios is equal to 1. As such, the resulting ratio for presence in CT could be taken as a relative index of occurrence (numerical prediction) [39].

> Many parameters in RF can be manipulated, e.g., the number of trees grown or the number of
variables to try at each split. Because the outcome of both CT and RT is not very sensitive to modifications of these parameters [31–33], it was deemed unnecessary to fine-tune all the RF models to their optimal capacity when they were used for a large number of species. Fine-tuning RF parameters may only be necessary when the method is used for a given well-known species.

> Summary: A detailed procedural framework was proposed for applying RF methods with presence-only data to model the distributions of species. Choosing RT rather than CT as the SDM is recommended if model discrimination capacity is viewed as more important than model reliability, and vice versa. MaxKappa, MaxOA, MinROCdist, and MaxTSS are four promising objective methods for binary conversion of continuous predictions when presence-only data are available. Numerical rather than binary prediction distribution maps are recommended, and binary conversion of model outputs should only be implemented when it is clearly justified by the application’s objective. This general procedural framework benefits the wholesale implementation involved with a large number of species because of its simplicity and flexibility. Due to the complexity of the ecosystem and the uniqueness of the life histories and physiological characteristics of the specific species, these general rules may not apply to all species [10]. Nevertheless, this procedural framework is meant to be a generic concept to be applied to virtually any model prediction question with presence-only data. Under the detailed framework developed in this study, for a specific species, the choice of RF (CT or RT) models can be determined by model performance metrics (model discrimination capacity and reliability). Fine-tuning RF parameters may also be necessary. Other methods that can deal with multicollinearity, autocorrelation, and binary conversions of numerical predictions can be integrated into this procedural framework. Although some SDMs are not very sensitive to collinearity and autocorrelation, they also can embrace these methods.


[3] A.H. Fielding, J.F. Bell, A review of methods for the assessment of prediction errors in conservation presence/absence
models, Environ. Conserv. 24 (1997) 38–49.
[4] C. Liu, M. White, G. Newell, Measuring and comparing the accuracy of species distribution models with presence–absence
data, Ecography 34 (2011) 232–243.
[5] M. Ballings, D. Van-den-Poel, AUC: Threshold Independent Performance Measures for Probabilistic Classifiers. R Package
Version 0.3.0, (2013) . https://CRAN.R-project.org/package=AUC.
[6] J. Pearce, S. Ferrier, Evaluating the predictive performance of habitat models developed using logistic regression, Ecol.
Modell. 133 (2000) 225–245.
[9] L. Zhang, F. Huettmann, S. Liu, P. Sun, Z. Yu, X. Zhang, C. Mi, Classification and regression with random forests as a standard
method for presence-only data SDMs: a future conservation example using China tree species, Ecol. Inform. 52 (2019) 46–56.
[10] L. Zhang, L. Wang, S. Liu, p. Sun, Z. Yu, S. Huang, X. Zhang, An evaluation of four threshold selection methods in species
occurrence modelling with random forest: case studies with Davidia involucrata and Cunninghamia lanceolata, Chin. J. Plant
Ecol. 41 (2017) 387–395.
[26] L. Breiman, Random forests, Mach. Learn. 45 (2001) 5–32.
[27] L. Zhang, L. Wang, X. Zhang, S. Liu, P. Sun, T. Wang, The basic principle of random forest and its applications in ecology— a
case study of Pinus yunnanensis, Acta Ecol. Sin. 34 (2014) 650–659.
[28] A. Liaw, M. Wiener, Classification and regression by randomForest, R News 2 (2002) 18–22.
[31] D.R. Cutler Jr., T.C.E, K.H. Beard, A. Cutler, K.T. Hess, J. Gibson, J.J. Lawler, Random forests for classification in ecology, Ecology
88 (2007) 2783–2792.
[32] J. Peters, B.D. Baets, N.E.C. Verhoest, R. Samson, S. Degroeve, P.D. Becker, W. Huybrechts, Random forests as a tool for
ecohydrological distribution modelling, Ecol. Model. 207 (2007) 304–318.
[33] M.S.O. Brieuc, C.D. Waters, D.P. Drinana, K.A. Naish, A practical introduction to random forest for genetic association studies
in ecology and evolution, Mol. Ecol. Resour. 18 (2018) 755–766.
[39] C. Strobl, J.D. Malley, G. Tutz, An introduction to recursive partitioning: Rationale, application and characteristics of
Classification and Regression Trees, Bagging and Random Forests, Psychol. Methods 14 (2009) 323–348.


### Literature Review: Random Forests Method with Presence-Only Data in Species’ Distribution Modeling

Zhang et al. (2019) expound on the power of the Random Forests (RF) as a Species Distribution Model (SDM) algorithm. At its core, RF is an ensemble learning technique designed to produce both categorical and numerical species distribution maps, a feat achieved using the classification tree (CT) and regression tree (RT) algorithms respectively (Breiman, 2001; Liaw & Wiener, 2002; Strobl et al., 2009). CT, in its statistical essence, also has the capability to provide probabilistic predictions, known as class probability (Zhang et al., 2019). The quintessence of RF lies in its bootstrap samples which are harnessed to construct a plethora of decision trees, which are then aggregated to predict new data. In the context of CT, model output is often categorical and determined by the proportion of votes, with the ratio for presence in CT potentially being leveraged as a relative index of occurrence or numerical prediction (Strobl et al., 2009).

With an abundance of metrics available for evaluating SDM performance, discerning the most effective one becomes challenging (Fielding & Bell, 1997; Liu et al., 2011). Different metrics invariably have their unique strengths, weaknesses, and methods of weighting prediction errors like omission, commission, or confusion. Emphasizing the importance of utilizing multiple performance metrics, Zhang et al. (2019) underscore the utility of threshold-independent evaluations such as root mean square error (RMSE), mean absolute prediction error (MAE), coefficient of determination (R^2), mean cross entropy (MXE), and several AUC measures (Ballings & Van-den-Poel, 2013). Numerical predictions, in particular, are distinguished by their discrimination capacity, such as AUC values, and reliability metrics, including RMSE, MAE, and R^2 (Liu et al., 2011; Pearce & Ferrier, 2000). The inherent importance of these two facets (reliability and discrimination capacity) is contingent upon the model's application and the user's proficiency level (Pearce & Ferrier, 2000).
