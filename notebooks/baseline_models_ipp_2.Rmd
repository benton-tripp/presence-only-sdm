...
plots.list <- purrr::map(bioclim.list, function(entry) {
  get.object({
    list(
    plot.raster.gg(entry$bioclim, "sum") + 
      labs(title = paste0("BIOCLIM Suitability for\n", entry$species, " in ",
                          entry$state),
           fill="BIOCLIM Sum"),
    plot.raster.gg(entry$bioclim < entry$qnt.5, "sum", F) + 
      labs(title = paste0("BIOCLIM Suitable sample regions for\n", entry$species, 
                          " in ", entry$state),
           fill="BIOCLIM Suitable\nPseudo-absence Region"),
    plot.raster.gg(entry$final.pseudo.absence.region, "sum", F) + 
      labs(title = paste0("Final Suitable sampleregions for\n", entry$species, 
                          " in ", entry$state),
           fill="Suitable Sampling\nRegions")
  ) %>% ggpubr::ggarrange(plotlist = ., ncol = 3)
  },
  paste0(entry$species, "_", entry$st, ".rds"),
  "artifacts/bioclim_suitable_plots")
})

# Arrange the plots
all.plots <- get.object({
  ap <- ggpubr::ggarrange(plotlist=plots.list, ncol=1)
  # Create a temporary file
  tmpfile <- tempfile(fileext = ".svg")
  # Save the ggplot to this file
  ggsave(filename = tmpfile, ap, width = 50, height=50)
...
paste0("combined_svg.rds"),
"artifacts/bioclim_suitable_plots")

 htmltools::div(
   style="overflow:auto; padding:5px;", 
   htmltools::tags$img(src=all.plots))
```
