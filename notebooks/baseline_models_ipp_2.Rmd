...
# Example
plot(purrr::detect(bioclim.list, ~.x$state == "VT" & 
             .x$species == "Wild Turkey")$bioclim, 
     main = "BIOCLIM Suitability for Wild Turkeys in Vermont")
```

```{r bioclim-ex-2}
# Example
qnt.5.wtvt <- quantile(values(purrr::detect(bioclim.list, ~.x$state == "VT" & 
                         .x$species == "Wild Turkey")$bioclim), .5, na.rm=T)
plot(purrr::detect(bioclim.list, ~.x$state == "VT" & 
             .x$species == "Wild Turkey")$bioclim < qnt.5.wtvt, 
     main = "BIOCLIM Suitable sample regions for Wild Turkeys in Vermont")
```
...
purrr::walk(1:nrow(spec.state), function(i) {
  ...
  bioclim.list[[idx]]$final.pseudo.absence.region <<-
    bioclim.list[[idx]]$bioclim <= qnt.5 & pa.region == 1
})
   

plot(purrr::detect(bioclim.list, ~.x$state == "VT" & 
             .x$species == "Wild Turkey")$final.pseudo.absence.region, 
     main = "Final Suitable sample regions for Wild Turkeys in Vermont") 
