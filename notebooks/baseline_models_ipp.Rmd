...
purrr::walk(1:nrow(spec.state), function(i) {
  ...
  if (!all(file.exists(c(glm.path, gam.path)))) {
    ...
    gam.pred <- spatstat.model::predict.ppm(fit.gam, 
                                            locations=locations.test[, .(x,y)],
                                            type="trend", se=T)
    gam.ci <- spatstat.model::predict.ppm(fit.gam, locations=locations.test[, .(x,y)],
                                          type="trend", interval="c")
    gam.test <- cbind(locations.test,
                      data.table(
                        est=gam.pred$estimate,
                        se=gam.pred$se# ,
                        # lo=gam.ci[1:length(gam.pred$estimate)],
                        # hi=gam.ci[(length(gam.pred$estimate) + 1):length(gam.ci)]
                      )
    )
    gam.test[, `:=` (
      p.obs = 1 - dpois(0, est)# , # probability > 0
    ...
    all.predictions.gam <- response.ppm(fit.gam)$window %>% as.matrix()
    gam.count.pred <- spatstat.model::predict.ppm(fit.gam, type="count", se=T)
    gam.pi <- spatstat.model::predict.ppm(fit.gam, type="count", 
                                          se=T, interval="p")
