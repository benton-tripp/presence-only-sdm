...
purrr::walk(1:nrow(spec.state), function(i) {
  ...
  if (!all(file.exists(c(glm.path, gam.path)))) {
    ...
    ipp.data <- prep.ipp.data(df.train, st, spec, r.list,
                       covariates=covariates)
    Q <- ipp.data$Q
    
    # Use pre-processing scaler on rasters
    to.scale <- covariates[ipp.data$scaled.covariates]
...
belted.kf.CO.glm <- readRDS(ipp.models[1, ]$glm.path)

# Diagnostics
bkf.co.glm.diag <- diagnose.ppm(belted.kf.CO.glm$fit, plot.it=F)
