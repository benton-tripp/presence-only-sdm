...
purrr::walk(1:nrow(spec.state), function(i) {
  ...
  if (!all(file.exists(c(glm.path, gam.path)))) {
    ...
    # TODO: Check for a spatial trend in residuals (Ripley's K-Function)
    ...
    # GLM Model
    fit.glm <- get.object(
      obj=ppm(Q=Q, trend=.f, covariates=r.funcs, covfunargs=.covfunargs, 
              method="mpl"), # 
    ...
    fit.gam <- get.object(
      obj=ppm(Q=Q, trend=.f, covariates=r.funcs, covfunargs=.covfunargs, 
              method="mpl", use.gam=T), # Method of Maximum PseudoLikelihood 
      # gcontrol=list() for additional glm/gam parameters
