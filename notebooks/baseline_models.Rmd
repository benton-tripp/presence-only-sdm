...
### TODO: Create NA Mask (e.g., sum(r.list$NC), then get all NA values)
...
redund.feat <- c("open_water", "developed_open_space", "developed_low_intensity", "developed_medium_intensity",
                 "developed_high_intensity", "hay_pasture", "cultivated_crops", "wetlands", "forest",
                 "lon.lat.interaction", "waterbody", "urban_imperviousness", "tmax", "tmin", "dem", "Fall_NDVI",
                 "Spring_NDVI", "Summer_NDVI", "Winter_NDVI")
...
# Reproject raster
r.reproject <- function(r, .crs=4326) {
  terra::project(x=r, 
                 y=st_crs(.crs, parameters=T)$Wkt)
}
...
# Function to split raster into n evenly distributed rectangles
split.raster <- function(r, n) {
  
  # Get raster dimensions
  ncols <- ncol(r)
  nrows <- nrow(r)
  
  # Determine the number of split rows and columns based on n
  # Using the sqrt as a starting point
  sqrt.n <- sqrt(n)
  
  if (sqrt.n == as.integer(sqrt.n)) {
    nsplit.rows <- nsplit.cols <- sqrt.n
  } else {
    aspect.ratio <- nrows / ncols
    nsplit.rows <- round(sqrt.n * aspect.ratio)
    nsplit.cols <- n / nsplit.rows
  }
  
  # Calculate the size of each split rectangle
  block.rows <- nrows / nsplit.rows
  block.cols <- ncols / nsplit.cols
  
  # Extract extents of each rectangle
  extents.list <- list()
  for (i in 1:nsplit.rows) {
    for (j in 1:nsplit.cols) {
      xmin <- (j - 1) * block.cols * xres(r) + xmin(r)
      xmax <- j * block.cols * xres(r) + xmin(r)
      ymin <- (i - 1) * block.rows * yres(r) + ymin(r)
      ymax <- i * block.rows * yres(r) + ymin(r)
      
      ext <- list(
        lon=c(xmin, xmax),
        lat=c(ymin, ymax)
      )
      extents.list[[length(extents.list) + 1]] <- ext
    }
  }
...
prep.ipp.data <- function(df, st, spec, 
                          raster.list,
                          bootstrap=F,
                          ntiles=12) {
  ...
  if (!bootstrap) {
    ...
    r.tmp <- r.reproject(raster.list[[st]])
    tile.extents <- split.raster(r.tmp, ntiles)
    locations <- purrr::map(tile.extents, function(rng) {
      ppp(ipp.df$lon, ipp.df$lat,
          window=owin(rng$lon, rng$lat))
    })
    locations
  }
}
...
cb.pcf <- purrr::map(1:nrow(spec.state), function(i) {
  ...
  locations <- prep.ipp.data(df.train, st, spec, r.list)
  loc.boot <- get.object(
...
if (!dir.exists(ipp.raster.dir)) dir.create(ipp.raster.dir)

ipp.models <- purrr::map(1:nrow(spec.state), function(i) {
  spec <- spec.state[i,]$species
  st <- spec.state[i,]$state
  ...
  ## TODO: 
  # 1) It is ok to have multiple observations at a single point. In fact, it might be good
  #    to include. Use an aggregation of all observation at a point, then reshape to 
  #    distinct rows. Then add "jitter" such that each point still exists within the 
  #    original raster cell.
  # 2) Similarly, for bootstrapping, you can sample with replacement. If the model 
  #    doesn't like duplicate points though, you might need to add "jitter" again
  #    prior to modeling. I.e., recompute the locations
  # 3) 
  
  r <- r.list[[st]]
  
  # Select rasters based on feature importance
  fs.df <- var.imp %>% 
  ...
  # 1. Check for a spatial trend in residuals (Ripley's K-Function)
  fit <- ppm(Q=.f, covariates=r.funcs, covfunargs=.covfunargs, 
             method="mpl") # Method of Maximum PseudoLikelihood
  # Fit the IPP model
  fit <- get.object(
    obj=ppm(Q=.f, covariates=r.funcs, covfunargs=.covfunargs, method="logi"), 
    # use.gam=T for general additive model
    # gcontrol=list() for additional glm/gam parameters
    file.name=paste0(spec, "_", st, "_ipp.rds"), 
    obj.path="artifacts/models/ipp"
    ) 
}, .progress=T)
...
### Bootstrap Confidence Bands for PCF
