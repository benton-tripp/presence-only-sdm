...
ipp.models <- purrr::map(1:nrow(spec.state), function(i) {
  ...
  r.dfs <- names(r)[!(names(r) %in% c("lat", "lon"))] %>%
    set_names() %>%
    purrr::map(function(.x) {
      cat("Generating dataframe for", .x, "\n")
      r[[.x]] %>% 
        # Scale the data
        min.max.scale() %>%
  ...
  r.funcs <- names(r.dfs) %>%
    ...
    purrr::map(function(.x) {
      ...
      arg_list <- setNames(list(quote(NULL), quote(NULL), quote(NULL)), c("x", "y", .x))
      
      # Create the body of the function
      body_expr <- substitute({
        filter(DATA_ARG, lon == x, lat == y)$z
      }, list(DATA_ARG = as.name(.x)))

      # Make the function
      f <- pryr::make_function(arg_list, body_expr)
      f
    })
  
  
  # Create formula
  .f <- formula(paste0("locations ~ ", paste(names(r), collapse = " + ")))
  # Fit the IPP model
  model <- ppm(Q=.f, covariates=r.funcs, covfunargs=r.dfs)
  fit <- get.model(
    model=ppm(Q=.f, covariates=r.imgs), 
    file.name=paste0(spec, "_", state, "_ipp.rds"), 
    model.path="artifacts/models/ipp"
    ) 
}, .progress=T)
