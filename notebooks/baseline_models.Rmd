...
lasso.model.path="artifacts/models/lasso_fs"
var.imp <- purrr::map_df(1:nrow(spec.state), function(i) {
  spec <- spec.state[i,]$species
  state <- spec.state[i,]$state
  fname <- file.path(lasso.model.path,
                     paste0(tolower(gsub(" ", "_", spec)), 
                            "_", state, "_regr_l1.rds"))
  fit <- readRDS(fname)
...
ipp.models <- purrr::map(1:nrow(spec.state), function(i) {
  ...
  r.ipp <- r.fs %>%
    ...
    purrr::map(function(.x) {
      cat("Generating pre-processed raster for", .x, "\n")
      r[[.x]] %>% 
        # Scale the data
        min.max.scale() %>%
