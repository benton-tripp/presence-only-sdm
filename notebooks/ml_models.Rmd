...
models <- purrr::map_df(c("logreg", "knn", "tree", "randomforest"), function(m) {
  m.type <- case_when(m == "logreg" ~ "logistic regression",
                      m == "knn" ~ "knn",
...
reshape.data <- function(data, metric) {
  ...
  data %>%
    ...
    tidyr::pivot_wider(names_from = model.type, 
                ...
                names_glue = "{model.type}.{tolower(metric)}")
}

metrics <- c("Accuracy", "Sensitivity", "Specificity", "F1") %>%
  set_names() %>%
  purrr::map(~{
    d <- reshape.data(models, .x)
    DT::datatable(
      d,
      filter='none',
      selection='none',
      ...
      DT::formatStyle(columns=names(d), 
                      `font-size`="13px") %>%
      DT::formatSignif(3:ncol(d), digits=2) %>%
      DT::renderDT() %>%
...
htmltools::div(
  ...
  htmltools::tags$select(id='metric_selector',
                         lapply(names(metrics), function(met) {
