...
shp.to.raster <- function(vector.ds, raster.fn, akey) {
  ...
  # Create the raster dataset
  rast.ds <- rast(nrows=size.y, ncols=size.x, xmin=ext[1], xmax=ext[2], ymin=ext[3], ymax=ext[4])
  proj <- crs(vec)
  crs(rast.ds) <- proj
  
  # Rasterize
  rast.ds <- rasterize(vec, rast.ds, field=akey, fun=max) # assuming we want to retain the max value where polygons overlap
  writeRaster(rast.ds, raster.fn, overwrite=TRUE)
}
...
musym.codes <- tibble(MUSYM = unique(gdf$MUSYM), MUSYM_CODE = 1:length(unique(gdf$MUSYM)))
...
st_write(gdf, "../data/soil/codes/gsmsoilmu_a_us.shp", delete_dsn = TRUE)
